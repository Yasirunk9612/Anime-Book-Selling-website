    ...
    if (mysqli_num_rows($select_product_name) > 0) {
        ...
        if ($image_size <= 2000000) {
            if (move_uploaded_file($image_tmp_name, $image_folder)) {
                ...
                $message[] = 'Temporary file path: ' . $_FILES['image']['tmp_name'];
                $message[] = 'Destination path: ' . $image_folder;
            }
        } else {
    ...
    if (!empty($update_image)) {
        if ($update_image_size <= 2000000) {
            if (move_uploaded_file($update_image_tmp_name, $update_folder)) {
                ...
                $message[] = 'Failed to move uploaded file.';
                $message[] = 'Upload error code: ' . $_FILES['update_image']['error'];
                $message[] = 'Temporary file path: ' . $_FILES['update_image']['tmp_name'];
                $message[] = 'Destination path: ' . $update_folder;
            }
        } else {
